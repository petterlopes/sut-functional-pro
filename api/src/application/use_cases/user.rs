...
impl<'a> UpdateUserUseCase<'a> {
    ...
    pub async fn execute(&self, request: UpdateUserRequest) -> Result<UserResponse, DomainError> {
        ...
        let mut user = self.user_repository.find_by_id(&user_id).await?
            .ok_or_else(|| DomainError::NotFound(format!("User with ID {} not found", request.id)))?;
        ...
        if let Some(email) = request.email {
            let email_vo = UserEmail::new(email)
                .map_err(|e| DomainError::ValidationError(e))?;
...
impl<'a> GetUsersUseCase<'a> {
    ...
    pub async fn execute(&self, request: UserSearchRequest) -> Result<UserSearchResponse, DomainError> {
    ...
    pub async fn execute_by_role(&self, role: &str) -> Result<UserSearchResponse, DomainError> {
        ...
        let items = users.into_iter().map(|user| user.into()).collect::<Vec<_>>();
