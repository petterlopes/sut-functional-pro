...
impl<'a> CreateDepartmentUseCase<'a> {
    pub fn new(department_repository: &'a dyn DepartmentRepository) -> Self {
        CreateDepartmentUseCase { department_repository }
    }

    pub async fn execute(&self, request: CreateDepartmentRequest) -> Result<DepartmentResponse, DomainError> {
        let unit_id = OrgUnitId(request.unit_id);
        let name = DepartmentName::new(request.name)
...
impl<'a> UpdateDepartmentUseCase<'a> {
    pub fn new(department_repository: &'a dyn DepartmentRepository) -> Self {
        UpdateDepartmentUseCase { department_repository }
    }

    pub async fn execute(&self, request: UpdateDepartmentRequest) -> Result<DepartmentResponse, DomainError> {
        ...
        let mut department = self.department_repository.find_by_id(&department_id).await?
            .ok_or_else(|| DomainError::NotFound(format!("Department with ID {} not found", request.id)))?;

        if let Some(name) = request.name {
            let department_name = DepartmentName::new(name)
                .map_err(|e| DomainError::ValidationError(e))?;
...
impl<'a> DeleteDepartmentUseCase<'a> {
    pub fn new(department_repository: &'a dyn DepartmentRepository) -> Self {
        DeleteDepartmentUseCase { department_repository }
    }
    ...
    pub async fn execute(&self, id: &str) -> Result<(), DomainError> {
        ...
        self.department_repository.find_by_id(&department_id).await?
...
impl<'a> GetDepartmentsUseCase<'a> {
    pub fn new(department_repository: &'a dyn DepartmentRepository) -> Self {
        GetDepartmentsUseCase { department_repository }
    }

    pub async fn execute(&self, request: DepartmentSearchRequest) -> Result<DepartmentSearchResponse, DomainError> {
        ...
        let items = result.items.into_iter().map(|department| department.into()).collect();
    ...
    pub async fn execute_by_id(&self, id: &DepartmentId) -> Result<DepartmentResponse, DomainError> {
        let department = self.department_repository.find_by_id(id).await?
            .ok_or_else(|| DomainError::NotFound(format!("Department with ID {} not found", id)))?;
        Ok(department.into())
    }

    pub async fn execute_by_unit(&self, unit_id: &OrgUnitId) -> Result<DepartmentSearchResponse, DomainError> {
        let departments = self.department_repository.find_by_unit(unit_id).await?;
        let items = departments.into_iter().map(|department| department.into()).collect::<Vec<_>>();
...
impl<'a> GetDepartmentStatisticsUseCase<'a> {
    pub fn new(department_repository: &'a dyn DepartmentRepository) -> Self {
        GetDepartmentStatisticsUseCase { department_repository }
    }
    ...
    pub async fn execute(&self) -> Result<DepartmentStatisticsResponse, DomainError> {
        ...
        let departments_by_unit = stats.departments_by_unit
            .into_iter()
            .map(|(k, v)| (k.0, v))
            .collect();
